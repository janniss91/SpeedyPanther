...
def fourier_analysis(sample_window: np.ndarray, duration: float = 0.005, sample_rate: int = 44_100):
    """
    ...
    assert int(sample_rate * duration) == n_samples, "The sample window and sample rate are not in line with each other."
    ...
    for freq in freq_bins:
        _, freq_samples = sample_sinewave(freq, end_time=duration, sample_rate=sample_rate)
